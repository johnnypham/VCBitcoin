...
namespace VCBitcoin {
    ...
    public static class Bech32 {
        ...
        private static byte[] createChecksum(string hrp, byte[] data, int dataLen) {
            ...
            Buffer.BlockCopy(data, 0, values, hrpLen, dataLen);
        ...
        private static bool verifyChecksum(string hrp, byte[] dataWithChecksum) {
            ...
            hrpExpand(hrp, values);
            Buffer.BlockCopy(dataWithChecksum, 0, values, hrp.Length * 2 + 1, dataWithChecksum.Length);
            return PolyMod(values) == 0;
        }

        private static int convertBits(byte[] bytes, int inBits, uint outBits, bool pad, 
            byte[] converted, int inPos, int outPos, int len) {
            ...
            if (pad) {
                ...
                throw new Bech32ConversionException(
                    "Bit conversion error!" + bits + " " + inBits + " " + 
                    ((val << (int)(outBits - bits)) & maxv));
            }
            return outPos;
        }
        ...
        public static string Encode(byte witnessVersion, byte[] witnessProgram, bool mainnet) {
            ...
            byte[] checksum;
            if (mainnet) {
                ret = new StringBuilder("bc", 75); ;
                checksum = createChecksum("bc", data, len);
            } else {
                ret = new StringBuilder("tb", 75); ;
        ...
        public static byte[] Decode(string addr, out byte witnessVersion, out byte isP2PKH, out bool mainnet) {
            ...
            Buffer.BlockCopy(decoded, 0, final, 0, decodedLen);
            return final;
        }
    }
}